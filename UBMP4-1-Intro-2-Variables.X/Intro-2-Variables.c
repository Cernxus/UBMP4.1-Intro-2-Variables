  ...
  while(1)
{
       ...
       // Reset count and turn off LED D5
       if(SW4 == 0)
       {
           LED5 = 0;
           SW2Count = 0;
...
- The maximum value it can store is around 255.
- The benifits of using an 8-bit variables ina n 8-bit microcontroller is that it can handle the 8-bit program.
- The drawback is that it limited to at most 8-bit.
...
- No, it did not reach 50, it takes about 4 times until LED D4 lights up.
- The programs sets a condition where it needs to reach a certain count in order for something to activate.
...
- While holding SW2 button, LED 4 would start blinking and when you let go of SW2,
  if LED D4 is still on then the moment you let go of SW2 the light would still stay on and stop blinking.
- Same goes for when LED D4 is off.
